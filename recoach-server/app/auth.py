...
本中间件只回答"谁能访问"，不回答"用户之间如何隔离"。因此共享令牌模式下，
**任何持令牌者都能用 `x-user-id` 指定任意身份**——这是刻意保留的单机单人模型，
不是实现失误（见 app/routes/sessions.py::current_user_id 的信任边界说明）。
需要收紧时开启 `RECOACH_LOCKED_USER`：身份被钉死为单一用户，
请求头指定他人会被拒绝而不是静默忽略。

多用户/公网部署仍应在反向代理层再做一层认证，并把令牌换成签名会话。
"""
